...
ENT.PrintName 		= "Forcefield (SHORT)"
...
function ENT:Initialize()
	...
	local tr = util.TraceLine({
		...
		endpos = self:GetPos() + self:GetRight() * -240,
		filter = function(ent) 
			if filter[ent] then
				return false
			end
...
function ENT:SetupDataTables()
	...
	self:NetworkVar("Bool", 2, "AllowVorts")
	self:NetworkVar("Bool", 3, "AllowWorkers")
	self:NetworkVar("Bool", 4, "AllowInfCon")
	self:NetworkVar("Bool", 5, "AllowPolice")
end

function ENT:CanPlayerPower(ent)
	if ent:Team() == TEAM_SCIENTIST or ent:Team() == TEAM_INSPECTORS or ent:Team() == TEAM_OVERWATCH or ent:Team() == TEAM_AIRWATCH then
	...
	if self:GetAllowCA() and (ent:Team() == TEAM_COMBINEADMIN or ent:Team() == TEAM_LABORADMIN) then
		return ent:HasCombinePermission(COMBINE_TECHNOLOGY_FORCEFIELD)
	end

	if self:GetAllowVorts() and ent:Team() == TEAM_VORTIGAUNT and ent:SpeciesState() == "slave" then
		return ent:HasCombinePermission(COMBINE_TECHNOLOGY_FORCEFIELD)
	end
...
function ENT:GetContextOptions(ply, interact)
	...
	if ply:IsInEditMode() then
		...
		table.insert(tab, {
            EditModeOnly = true,
			Name = self:GetAllowVorts() and "Restrict Servitor Passage" or "Allow Servitor Passage",
			Callback = function()
				self:SetAllowVorts(not self:GetAllowVorts())
				self:Save()
			end
		})
...
hook.Add("ShouldCollide", "forcefield.ShouldCollide.Short", function(a, b)
	local field
	local ent

	if IsValid(a) and a:GetClass() == "ent_combineforcefield_short" then
		field = a
		ent = b
	elseif IsValid(b) and b:GetClass() == "ent_combineforcefield_short" then
		field = b
		ent = a
	end
...
if SERVER then
	...
	function ENT:LoadCustomData(data)
		...
		self:SetAllowVorts(data.AllowVorts)
